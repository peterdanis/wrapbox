...
EventElement.prototype.unbindAll = function unbindAll() {
  ...
  for (const name in this$1.handlers) {
    this$1.unbind(name);
  }
};
...
prototypeAccessors.isEmpty.get = function () {
  ...
  return Object.keys(this.handlers).every(key => this$1.handlers[key].length === 0);
};
...
const processScrollDiff = function (i, axis, diff, useScrollingClass, forceFireReachEvent) {
...
const updateGeometry = function (i) {
  ...
  if (!element.contains(i.scrollbarXRail)) {
    // clean up and append
    queryChildren(element, cls.element.rail("x")).forEach(el => remove(el));
    element.appendChild(i.scrollbarXRail);
  }
  if (!element.contains(i.scrollbarYRail)) {
    // clean up and append
    queryChildren(element, cls.element.rail("y")).forEach(el => remove(el));
  ...
  if (
    ...
    i.scrollbarYTop = toInt(element.scrollTop *
...
const PerfectScrollbar = function PerfectScrollbar(element, userSettings) {
  ...
  this.reach = {
    x:
      element.scrollLeft <= 0
        ? "start"
        : element.scrollLeft >= this.contentWidth - this.containerWidth ? "end" : null,
    y:
      element.scrollTop <= 0
        ? "start"
        : element.scrollTop >= this.contentHeight - this.containerHeight ? "end" : null,
  };

  this.isAlive = true;

  this.settings.handlers.forEach(handlerName => handlers[handlerName](this$1));

  this.lastScrollTop = element.scrollTop; // for onScroll only
  this.lastScrollLeft = element.scrollLeft; // for onScroll only
  this.event.bind(this.element, "scroll", e => this$1.onScroll(e));
  updateGeometry(this);
};
